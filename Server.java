...
public class Server {
	...
	static BigInteger nD, dD, messD, dMes; //These are the variables for the encryption

	static BigInteger p, q, n, e, d, T, mess; //These are the variable for the decryption

	static int len = 1024;

	static byte[] RSA_en(byte[] message) {// RSA key generation and encryption
		
		byte tempRes[]= new byte[768];
		Random R=new Random();//For Key Generation.
		
	        
			p = new BigInteger(""); //Random prime number value is chosen for p, p=BigInteger.probablePrime(len,R)  
		
		q = new BigInteger(""); //Random prime number value is chosen for q, q=BigInteger.probablePrime(len,R)

	
		e = new BigInteger("12652604729380209029747565797335916011689725878865771937862336854685648337926826365446638178344722550931417527336888606604951614951006272476369610354067401"); // e is chosen that has no common factors and relatively prime, e=BigInteger.probablePrime(512,R)
		
		d = new BigInteger(""); //d is the multiplicative inverse, d=e.modInverse(T)
			n=p.multiply(q);	// n= p*q
	    mess = (new BigInteger(message)).modPow(e, n);// RSA Encryption 
		...
		dMes = messD.modPow(dD, nD);// RSA Decryption
	...
	public static void main(String[] args) {
		...
		System.out.println("Alice"); //Starting the server
		try {
			DatagramSocket serverSocket = new DatagramSocket(1556);
			byte[] receiveData = new byte[768]; //message(256)+ n(256) +d(256)
			byte[] sendMsg;
			...
			while (true) {
				...
				System.out.println("Alice waiting for text"); //server is ready to accept data

				serverSocket.receive(receivePacket);

				System.out.println("Message received..."); //packet is received

				System.out.println("From Bob : " + new String(RSA_Dcr(receivePacket.getData()))); //bob here will be the client
